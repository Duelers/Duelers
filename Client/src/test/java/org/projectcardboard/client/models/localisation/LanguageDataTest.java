...
public class LanguageDataTest {
    ...
    /**
     * Until we make our language loading a bit more static, any new fields should
     * be added to this. It's the closest we can get right now to making sure at
     * least the default will work around compile time.
     */
    @Test
    public void loadDataDefaultMustWork() {
        LanguageData recieved = LanguageData.getInstance();

        assertExisted(recieved.getValue(new String[] { LanguageKeys.LOGIN_MENU, LanguageKeys.WELCOME_MESSAGE }));
        ...
        assertExisted(recieved.getValue(new String[] { LanguageKeys.LOGIN_MENU, LanguageKeys.ERROR_SHORT_USERNAME }));
        assertExisted(recieved.getValue(new String[] { LanguageKeys.LOGIN_MENU, LanguageKeys.ERROR_SHORT_PASSWORD }));

        assertExisted(recieved.getValue(new String[] { LanguageKeys.MAIN_MENU, LanguageKeys.PLAY }));
        assertExisted(recieved.getValue(new String[] { LanguageKeys.MAIN_MENU, LanguageKeys.COLLECTION }));
        ...
        assertExisted(recieved.getValue(new String[] { LanguageKeys.SPECTATE_MENU, LanguageKeys.ONLINE_GAMES }));

        assertExisted(recieved.getValue(new String[] { LanguageKeys.PROFILE_MENU, LanguageKeys.MATCH_HISTORY }));
