...
public class MapBox implements PropertyChangeListener {
  ...
  private static Logger logger = LoggerFactory.getLogger(MapBox.class);
  ...
  private void makePolygons() {
    for (int j = 0; j < GameMap.getNumRows(); j++) {
      double upperWidth =
          (j * Constants.MAP_DOWNER_WIDTH + (6 - j) * Constants.MAP_UPPER_WIDTH) / 6;
      double downerWidth =
          ((j + 1) * Constants.MAP_DOWNER_WIDTH + (6 - (j + 1)) * Constants.MAP_UPPER_WIDTH) / 6;
    ...
    if (newTroop == null) {
      ...
      try {
        animation = new TroopAnimation(mapGroup, cellsX, cellsY, newTroop.getCard().getSpriteName(),
            ...
            newTroop.getCard().isCustom());
        animation.updateApHp(newTroop.getCurrentAp(), newTroop.getCurrentHp());
        animation.getTroopGroup()
            .setOnMouseClicked(mouseEvent -> clickCell(animation.getRow(), animation.getColumn()));
        animation.getTroopGroup()
            .setOnMouseEntered(mouseEvent -> hoverCell(animation.getRow(), animation.getColumn()));
        animation.getTroopGroup()
            .setOnMouseExited(mouseEvent -> exitCell(animation.getRow(), animation.getColumn()));
        troopAnimationHashMap.put(newTroop, animation);
      } catch (Exception e) {
  ...
  private void clickCell(int row, int column) {
    ...
    if (selectionType.equals(SelectionType.INSERTION)) {
      if (GameController.getInstance().getAvailableActions()
          .canInsertCard(battleScene.getHandBox().getSelectedCard())) {
    ...
    if (selectionType.equals(SelectionType.SELECTION)) {
      if (currentTroop != null
          && currentTroop.getPlayerNumber() == battleScene.getMyPlayerNumber()) {
  ...
  private Boolean updateMapColoursOnNormal(int row, int column, Player player) {
    ...
    boolean canAttack = GameController.getInstance().getAvailableActions().canAttack(gameMap,
        player, selectedTroop, row, column);
    boolean canMove = GameController.getInstance().getAvailableActions().canMove(gameMap, player,
        selectedTroop, row, column);
  ...
  void showAttack(String cardId, String defender) {
    ...
    if (cardId == null || defender == null) {
      logger.info("CardId and/or Defender is null");
      return;
    }
    ...
    if (troop == null || defenderTroop == null) {
      logger.info("(show attack) attacking troop and/or defending troop is null");
      return;
    }

    TroopAnimation animation = troopAnimationHashMap.get(troop);
    if (animation == null) {
      logger.error("(show attack) attack animation is null");
    }
  ...
  void showDefend(String defender, String attacker) {
    ...
    if (troop == null || attackerTroop == null) {
      logger.info("(show defend) attacking troop and/or defending troop is null");
      return;
    }

    TroopAnimation animation = troopAnimationHashMap.get(troop);
    if (animation == null) {
      logger.error("(show defend) attack animation is null");
      return;
    }
