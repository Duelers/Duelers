...
public class PlayerBox implements PropertyChangeListener {
  ...
  private final Image inActiveManaImage =
      new Image(this.getClass().getResourceAsStream("/ui/icon_mana_inactive@2x.png"));
  private final Image chatImage =
      new Image(this.getClass().getResourceAsStream("/ui/chat_bubble.png"));
  ...
  private HashMap<String, String> MapGeneralToPortrait() {
    // Note that this is a quick hack; a better solution is to have "portraitId" defined in the
    // Hero's json file.
  ...
  private void addMuteButton() {
    this.muteButton =
        new OrangeButton("Disable Chat", event -> handleMuteButtonOnClick(), null, false);
  ...
  private void addHeroPortraits() throws FileNotFoundException {
    ...
    Image player1Profile = new Image(this.getClass().getResourceAsStream(MapGeneralToPortrait()
        .getOrDefault(player1HeroName, "/photo/general_portrait_image_hex_rook@2x.png")));
    ...
    player2Image = ImageLoader.makeImageView(player2Profile,
        player2Profile.getWidth() * SCALE * 0.3, player2Profile.getHeight() * SCALE * 0.3);
    ...
    DefaultLabel player1Name =
        new DefaultLabel("", Constants.NAME_FONT, Color.WHITE, 290 * SCALE, 75 * SCALE);
    player1Name.setBackground(new Background(
        new BackgroundFill(Color.rgb(155, 82, 100, 0.0), new CornerRadii(3), Insets.EMPTY)));
    player1Name.setPadding(Constants.NAME_PADDING);
    DefaultLabel player2Name = new DefaultLabel("", Constants.NAME_FONT, Color.WHITE,
        SCREEN_WIDTH - 500 * SCALE, 75 * SCALE);
    player2Name.setBackground(new Background(
        new BackgroundFill(Color.rgb(155, 82, 100, 0.0), new CornerRadii(3), Insets.EMPTY)));
  ...
  class MessageShow extends AnimationTimer {
    ...
    private final DefaultText text =
        new DefaultText("", CHAT_BUBBLE_SIZE * 0.9, UIConstants.DEFAULT_FONT, Color.BLACK);
