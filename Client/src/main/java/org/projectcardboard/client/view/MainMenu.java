...
public class MainMenu extends Show {
  ...
  private final String playText =
      LanguageData.getInstance().getValue(new String[] {LanguageKeys.MAIN_MENU, LanguageKeys.PLAY});
  private final String collectionText = LanguageData.getInstance()
      .getValue(new String[] {LanguageKeys.MAIN_MENU, LanguageKeys.COLLECTION});
  private final String chatText =
      LanguageData.getInstance().getValue(new String[] {LanguageKeys.MAIN_MENU, LanguageKeys.CHAT});
  private final String spectateText = LanguageData.getInstance()
      .getValue(new String[] {LanguageKeys.MAIN_MENU, LanguageKeys.SPECTATE});
  private final String profileText = LanguageData.getInstance()
      .getValue(new String[] {LanguageKeys.MAIN_MENU, LanguageKeys.PROFILE});
  private final String quitText =
      LanguageData.getInstance().getValue(new String[] {LanguageKeys.MAIN_MENU, LanguageKeys.QUIT});

  private final String logoutText = LanguageData.getInstance()
      .getValue(new String[] {LanguageKeys.PROFILE_MENU, LanguageKeys.LOGOUT});

  private final String onlineGamesText = LanguageData.getInstance()
      .getValue(new String[] {LanguageKeys.SPECTATE_MENU, LanguageKeys.ONLINE_GAMES});

  private static final Media backgroundMusic =
      new Media(MainMenu.class.getResource("/music/main_menu.m4a").toString());
  ...
  private void showOnlineGamesList() {
    ...
    dialogBox.makeClosable(onlineGamesDialog,
        closeEvent -> BackgroundMaker.makeMenuBackgroundUnfrozen());
