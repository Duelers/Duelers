...
import org.projectcardboard.client.controller.GameResultController;
...
import static org.projectcardboard.client.controller.SoundEffectPlayer.SoundName.victory_reward;
...
public class GameResultMenu extends Show {
  ...
  private static final Font RESULT_FONT =
      Font.font("DejaVu Sans Light", FontWeight.EXTRA_LIGHT, 100 * SCALE);
  private static final Font MIDDLE_FONT =
      Font.font("DejaVu Sans Light", FontWeight.EXTRA_LIGHT, 70 * SCALE);

  static {
    results.put(true,
        new ResultData("scene_diamonds_background_victory@2x",
            "scene_diamonds_middleground_victory@2x", "scene_diamonds_foreground_friendly@2x",
            "general_winner", ProfileGrid.goldIcon, "VICTORY", Color.rgb(115, 210, 255)));
    results.put(false,
        new ResultData("scene_diamonds_background_defeat@2x",
            "scene_diamonds_middleground_defeat@2x", "scene_diamonds_foreground_enemy@2x",
            "general_loser", ProfileGrid.generalIcon, "FAILURE", Color.rgb(255, 100, 115)));
  }
  ...
  public GameResultMenu() {
    ...
    try {
      ...
      ImageView resultBackground =
          ImageLoader.makeImageView(data.background, SCENE_WIDTH, SCENE_HEIGHT);
      ImageView resultMiddleground =
          ImageLoader.makeImageView(data.middleground, MIDDLEGROUND_WIDTH, MIDDLEGROUND_HEIGHT);
      ...
      resultLabel
          .setEffect(new DropShadow(BlurType.THREE_PASS_BOX, data.color, 100 * SCALE, 0.7, 0, 0));

      ImageButton playMenuButton =
          new ImageButton("PLAY MENU", event -> PlayMenu.getInstance().show());
      ...
      ImageView resultForeground = ImageLoader.makeImageView(
          results.get(GameResultController.getInstance().amInWinner()).foreground, SCENE_WIDTH,
          SCENE_HEIGHT);

      AnchorPane sceneContents = new AnchorPane(background, resultBackground, resultMiddleground,
  ...
  private static class ResultData {
    ...
    private ResultData(String backName, String middleName, String foreName, String heroName,
        Image middleIcon, String message, Color color) {
