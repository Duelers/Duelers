...
class MatchHistoryTable extends ScrollPane {
    ...
    private static final Background BACKGROUND = new Background(
            new BackgroundFill(Color.rgb(57, 63, 74, 0.5), new CornerRadii(BACKGROUND_CORNER), Insets.EMPTY));
    ...
    MatchHistoryTable(List<MatchHistory> matchHistories) {
        ...
        for (int i = 0; i < matchHistories.size(); i++) {
            ...
            Label stateLabel = new DefaultLabel(history.getAmIWinner() ? "Win" : "Lose", UIConstants.DEFAULT_FONT,
                    Color.WHITE);
            ...
            labelsGrid.addRow(2 * i, opponentLabel, new DefaultSeparator(Orientation.VERTICAL, SEPARATOR_OPACITY),
                    dateLabel, new DefaultSeparator(Orientation.VERTICAL, SEPARATOR_OPACITY), stateLabel,
                    new DefaultSeparator(Orientation.VERTICAL, SEPARATOR_OPACITY), showButton);
            if (i < matchHistories.size() - 1) {
                labelsGrid.add(new DefaultSeparator(Orientation.HORIZONTAL, SEPARATOR_OPACITY), 0, 2 * i + 1, 7, 1);
            }
        }
