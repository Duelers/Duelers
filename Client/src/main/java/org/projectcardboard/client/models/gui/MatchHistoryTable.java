...
import org.projectcardboard.client.models.account.MatchHistory;
...
class MatchHistoryTable extends ScrollPane {
  ...
  private static final Background BACKGROUND =
      new Background(new BackgroundFill(Color.rgb(57, 63, 74, 0.5),
  ...
  MatchHistoryTable(List<MatchHistory> matchHistories) {
    ...
    for (int i = 0; i < matchHistories.size(); i++) {
      ...
      Label opponentLabel =
          new DefaultLabel(history.getOppName(), UIConstants.DEFAULT_FONT, Color.WHITE);
      opponentLabel.setPadding(new Insets(UIConstants.DEFAULT_SPACING));

      Label dateLabel =
          new DefaultLabel(history.getDate().toString(), UIConstants.DEFAULT_FONT, Color.WHITE);
      dateLabel.setPadding(new Insets(UIConstants.DEFAULT_SPACING));

      Label stateLabel = new DefaultLabel(history.getAmIWinner() ? "Win" : "Lose",
      ...
      labelsGrid.addRow(2 * i, opponentLabel,
          new DefaultSeparator(Orientation.VERTICAL, SEPARATOR_OPACITY), dateLabel,
          new DefaultSeparator(Orientation.VERTICAL, SEPARATOR_OPACITY), stateLabel,
          new DefaultSeparator(Orientation.VERTICAL, SEPARATOR_OPACITY), showButton);
      if (i < matchHistories.size() - 1) {
        labelsGrid.add(new DefaultSeparator(Orientation.HORIZONTAL, SEPARATOR_OPACITY), 0,
            2 * i + 1, 7, 1);
      }
    }
