...
public class Account extends BaseAccount<Deck, Collection, MatchHistory> {
  ...
  public boolean equals(Object obj) {
    if (obj.getClass().getName().equals(AccountInfo.class.getName())) {
      ...
      if (this.username.equals(accountInfo.getUsername()))
        return true;
    }
    if (!obj.getClass().getName().equals(Account.class.getName()))
      return false;
  ...
  public void update(TempAccount account) { // Todo this stops all fields from being final. It
                                            // should make a new account instead.
  ...
  private boolean mainDecksEqual(TempAccount account) {
    return ((mainDeck == null && account.getMainDeckName() == null)
        || (mainDeck != null && account.getMainDeckName() != null
            && mainDeck.getName().equals(account.getMainDeckName())));
  }

  private boolean decksEqual(List<TempDeck> decks) {
    if (this.decks.size() != decks.size())
      return false;
