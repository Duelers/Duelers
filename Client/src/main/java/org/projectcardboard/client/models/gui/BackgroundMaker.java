...
public class BackgroundMaker {
  ...
  private static void makeMenuBackground() throws FileNotFoundException {
    ...
    ImageView backgroundView =
        ImageLoader.loadImage(BACKGROUND_URL, UIConstants.SCENE_WIDTH, UIConstants.SCENE_HEIGHT);
    ImageView foregroundView = ImageLoader.loadImage(FOREGROUND_URL, FOREGROUND_WIDTH,
        FOREGROUND_HEIGHT, UIConstants.SCENE_WIDTH - FOREGROUND_WIDTH,
        UIConstants.SCENE_HEIGHT - FOREGROUND_HEIGHT);
    ImageView farPillarsView = ImageLoader.loadImage(FAR_PILLAR_URL, FAR_PILLARS_WIDTH,
        FAR_PILLARS_HEIGHT, 0, UIConstants.SCENE_HEIGHT - FAR_PILLARS_HEIGHT);
    ImageView nearPillarsView = ImageLoader.loadImage(NEAR_PILLAR_URL, NEAR_PILLARS_WIDTH,
        NEAR_PILLARS_HEIGHT, 0, UIConstants.SCENE_HEIGHT - NEAR_PILLARS_HEIGHT);
    ImageView vignetteView = ImageLoader.loadImage(VIGNETTE_URL, VIGNETTE_WIDTH, VIGNETTE_HEIGHT);

    Cloud cloud = new Cloud();

    menuBackground.getChildren().addAll(backgroundView, farPillarsView, nearPillarsView, cloud,
        foregroundView, vignetteView);
  }
  ...
  private static void makePlayBackground(String url) throws FileNotFoundException {
    ...
    ImageView backgroundView =
        ImageLoader.loadImage(url, UIConstants.SCENE_WIDTH, UIConstants.SCENE_HEIGHT);
