...
public class GameServer {
  ...
  private static void sendOriginalCards(Message message) throws LogicException {
    DataCenter.getInstance().loginCheck(message);
    sendMessageAsync(Message.makeOriginalCardsCopyMessage(message.getSender(),
        DataCenter.getInstance().getOriginalCards()));
  }


  private static void selectUserForMultiPlayer(Message message) throws ClientException {
    Account account =
        DataCenter.getInstance().getAccount(message.getNewGameFields().getOpponentUsername());
    if (account == null) {
  ...
  public void sendChangeCardPositionMessage(Game game, ServerCard card,
  ...
  public void sendAttackMessage(Game game, ServerTroop attacker, ServerTroop defender,
  ...
  public void sendGameUpdateMessage(Game game) {
    ...
    for (Account account : game.getObservers()) {
      ...
      sendMessageAsync(Message.makeGameUpdateMessage(clientName, game.getTurnNumber(),
          game.getPlayerOne().getCurrentMP(), game.getPlayerTwo().getCurrentMP(), cellEffects));
    }
  }
  ...
  public void sendGameFinishMessages(Game game) {
    for (Account account : game.getObservers()) {
      ...
      if (account.getUsername().equals(game.getPlayerOne().getUserName())) {
        sendMessageAsync(Message.makeGameFinishMessage(clientName,
            game.getPlayerOne().getMatchHistory().getAmIWinner()));
        sendMessageAsync(Message.makeAccountCopyMessage(clientName,
            DataCenter.getInstance().getAccount(game.getPlayerOne().getUserName())));
      } else if (account.getUsername().equals(game.getPlayerTwo().getUserName())) {
        sendMessageAsync(Message.makeGameFinishMessage(clientName,
            game.getPlayerTwo().getMatchHistory().getAmIWinner()));
        sendMessageAsync(Message.makeAccountCopyMessage(clientName,
            DataCenter.getInstance().getAccount(game.getPlayerTwo().getUserName())));
      } else {
        sendMessageAsync(Message.makeGameFinishMessage(clientName, false));
      }
    }
  }
  ...
  public void sendChangeCardNumberMessage(ServerCard card) {
    for (Account account : DataCenter.getInstance().getAccounts().keySet()) {
      if (account.getAccountType().equals(AccountType.ADMIN)
        ...
        sendMessageAsync(Message.makeChangeCardNumberMessage(
            DataCenter.getInstance().getAccounts().get(account), card, card.getRemainingNumber()));
      }
    }
  }
    ...
    for (Account account : DataCenter.getInstance().getAccounts().keySet()) {
      if (DataCenter.getInstance().isOnline(account.getUsername())) {
        sendMessageAsync(Message
            .makeAddOriginalCardMessage(DataCenter.getInstance().getAccounts().get(account), card));
      }
    }
  }
  ...
  public void sendCardsDrawnToHandMessage(Game game, int deckSize, ServerCard... drawnCards) {
    for (Account account : game.getObservers()) {
      String clientName = DataCenter.getInstance().getAccounts().get(account);
      if (clientName == null) {
        serverPrint("*Error: Failed to get client name of account " + account.getUsername()
            + ". Deleting from list of observers.");
