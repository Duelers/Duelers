...
public class GameCenter extends Thread {// synchronize
  ...
  private void addUserInvitation(Account inviter, Account invited, GameType gameType) {
    ...
    synchronized (userInvitations) {
      ...
      GameServer.sendMessageAsync(Message.makeInvitationMessage(
          DataCenter.getInstance().getAccounts().get(invited), inviter.getUsername(), gameType));
    }
  }
  ...
  public void getAcceptRequest(Message message) throws LogicException {
    ...
    synchronized (userInvitations) {
      if (message.getNewGameFields() == null
      ...
      GameServer.sendMessageAsync(
          Message.makeAcceptRequestMessage(DataCenter.getInstance().getAccounts().get(inviter)));
  ...
  public void getDeclineRequest(Message message) throws LogicException {
    ...
    synchronized (userInvitations) {
      ...
      userInvitations.remove(invitation);
      GameServer.sendMessageAsync(
          Message.makeDeclineRequestMessage(DataCenter.getInstance().getAccounts().get(inviter)));
    }
  }
  ...
  private void newMultiplayerGame(Account account1, Account account2, GameType gameType) {
    ...
    GameServer.sendMessageAsync(Message
        .makeGameCopyMessage(DataCenter.getInstance().getClientName(account1.getUsername()), game));
    GameServer.sendMessageAsync(Message
        .makeGameCopyMessage(DataCenter.getInstance().getClientName(account2.getUsername()), game));
    game.startGame();
  }
  ...
  public void insertCard(Message message) throws LogicException {
    ...
    game.insert(DataCenter.getInstance().getClients().get(message.getSender()).getUsername(),
  ...
  public void attack(Message message) throws LogicException {
    Game game = getGame(message.getSender());
    game.attack(DataCenter.getInstance().getClients().get(message.getSender()).getUsername(),
  ...
  public void moveTroop(Message message) throws LogicException {
    Game game = getGame(message.getSender());
    game.moveTroop(DataCenter.getInstance().getClients().get(message.getSender()).getUsername(),
        message.getOtherFields().getMyCardId(), message.getOtherFields().getCell());
  }
