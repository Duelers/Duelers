...
public class TargetData {
  ...
  public Set<Cell> getPositions() {
    Set<Cell> cells = this.cells.stream().map(n -> new Cell(n.getRow(), n.getColumn()))
        .collect(Collectors.toSet());
    cells.addAll(troops.stream().map(ServerTroop::getCell)
        .map(n -> new Cell(n.getRow(), n.getColumn())).collect(Collectors.toSet()));
