...
public class AvailableActions extends
    BaseAvailableActions<Insert, Attack, Move, ServerTroop, GameMap, Game, ServerCard, Player> {
  ...
  public void calculateAvailableAttacks(Game game) {
    ...
    for (ServerTroop myTroop : ownPlayer.getTroops()) {
      ...
      for (ServerTroop enemyTroop : otherPlayer.getTroops()) {
        if (enemyTroop.canBeAttackedFromWeakerOnes()
            && myTroop.getCurrentAp() < enemyTroop.getCurrentAp())
          continue;

        if (checkRangeForAttack(myTroop, enemyTroop))
          continue;
