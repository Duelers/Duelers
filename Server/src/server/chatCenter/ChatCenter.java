...
public class ChatCenter {
  ...
  public void getMessage(Message message) {
    if (message.getChatMessage().getReceiverUsername() == null) {
      for (Account account : DataCenter.getInstance().getAccounts().keySet()) {// TODO:can has much
                                                                               // more performance
      ...
      if (message.getChatMessage().getReceiverUsername().equalsIgnoreCase("AI")) {
        new Thread(() -> {
          ...
          sendMessage(
              DataCenter.getInstance().getClientName(message.getChatMessage().getSenderUsername()),
              "AI", message.getChatMessage().getSenderUsername(), "Shut UP :)");
        }).start();
      } else if (DataCenter.getInstance()
          .isOnline(message.getChatMessage().getReceiverUsername())) {
        sendMessage(
            DataCenter.getInstance().getClientName(message.getChatMessage().getReceiverUsername()),
            message.getChatMessage().getSenderUsername(),
            message.getChatMessage().getReceiverUsername(), message.getChatMessage().getText());
      }
    }

  }

  public void sendMessage(String receiverClientName, String senderUsername, String receiverUsername,
      String text) {
    if (receiverClientName == null) {
      GameServer.serverPrint("Chat Receiver Error!");
    }
    Message message =
        Message.makeChatMessage(receiverClientName, senderUsername, receiverUsername, text);
