...
public class DataCenter extends Thread {
  ...
  private static final String[] CARDS_PATHS =
      {"Server/resources/heroCards", "Server/resources/minionCards", "Server/resources/spellCards"};
    ...
    if (!ClientPortal.getInstance().hasThisClient(client)) {
      ...
      if (!account.getCollection().equals(dataBase.getOriginalCards())) {
        System.out.println("Account " + account.getUsername()
            + " does not own all cards in current collection. Updating...");
  ...
  private void register(String username, String client) throws LogicException {
    if (username == null || username.length() < 2 || getAccount(username) != null) {
  ...
  public void addToDeck(Message message) throws LogicException {
    loginCheck(message);
    Account account = clients.get(message.getSender());
    account.addCardToDeck(message.getOtherFields().getMyCardId(),
        message.getOtherFields().getDeckName());
    GameServer.sendMessageAsync(Message.makeAccountCopyMessage(message.getSender(), account));
    saveAccount(account);
  }
  ...
  public void removeFromDeck(Message message) throws LogicException {
    loginCheck(message);
    Account account = clients.get(message.getSender());
    account.removeCardFromDeck(message.getOtherFields().getMyCardId(),
        message.getOtherFields().getDeckName());
    GameServer.sendMessageAsync(Message.makeAccountCopyMessage(message.getSender(), account));
    saveAccount(account);
  }
  ...
  public void changeCardNumber(Message message) throws LogicException {
    ...
    changeCardNumber(message.getChangeCardNumber().getCardName(),
  ...
  public void saveAccount(Account account) {
    ...
    try {
      FileWriter writer =
          new FileWriter(ACCOUNTS_PATH + "/" + account.getUsername() + ".account.json");
